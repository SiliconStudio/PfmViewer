...
int main(int argc, char* argv[])
{
    ...
    *in >> pfm.magic >> pfm.w >> pfm.h >> pfm.scale_endian;
    ...
    if (alloc == 0)
    {
        msgbox m({}, "No data", msgbox::ok);
        m.icon(m.icon_information);
        m << "Width and height are 0 or not found";
        m.show();
        return 2;
    }
    if (alloc > 1'000'000'000)
    {
        msgbox m({}, "Calculated image size too large", msgbox::ok);
        m.icon(m.icon_error);
        m << "More than 1GiB of data needed because of parsed width:" << pfm.w << " and height:" << pfm.h;
        m.show();
        return 3;
    }
